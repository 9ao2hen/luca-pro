...
import cn.mervyn.luca.module.report.controller.admin.goview.vo.data.GoViewDataRespVO;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("墨文源码").thenReturn("墨文");
        ...
        assertEquals("墨文源码", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("墨文", dataBySQL.getSource().get(1).get("name"));
    }
